...
static inline bool audio_is_usb_device(audio_devices_t device)
{
    if ((device & AUDIO_DEVICE_BIT_IN) != 0) {
        device &= ~AUDIO_DEVICE_BIT_IN;
        if ((popcount(device) == 1) && (device & AUDIO_DEVICE_IN_ALL_USB))
            return true;
        else
            return false;
    }
...
static inline uint32_t audio_channel_out_count_from_mask(audio_channel_mask_t channel_mask)
{
    switch(channel_mask) {
    ...
    case (AUDIO_CHANNEL_OUT_STEREO | AUDIO_CHANNEL_OUT_FRONT_CENTER):
        return 3;
    case AUDIO_CHANNEL_OUT_QUAD: // 4.0
        return 4;
    case (AUDIO_CHANNEL_OUT_QUAD | AUDIO_CHANNEL_OUT_FRONT_CENTER): // 5.0
        return 5;
    case AUDIO_CHANNEL_OUT_5POINT1: // 5.1
        return 6;
    case (AUDIO_CHANNEL_OUT_5POINT1 | AUDIO_CHANNEL_OUT_BACK_CENTER): // 6.1
        return 7;
